...
from yunBao_auto.Public import operation,DbConnect
from yunBao_auto.page import comMethod, HomePage_1, accountManage
...
browserPage = HomePage_1.BrowserDriver()
class MeiXing(unittest.TestCase):
    u"""美行"""
    def test_Login(self):
        u"""登陆"""
        #打开浏览器，并登陆
        #browserPage.getHomePage()
        #HomePage.BrowserDriver()
        print 'already opened when inialzing in the operation.py'
        #登陆渠道
    ...
    def delete(self):
        try:
            sql1="DELETE from t_yb_system_user WHERE account='ziaccount'"
            db = 'yb'
            dbconn.sqlExec(sql1,db)
        except:
            log.debug("删除数据失败")
